...
- `update_body_patches` (Attributes List) The body patches for update only. Any change here won't cause a plan diff. (see [below for nested schema](#nestedatt--update_body_patches))
...
- `path` (String) The path (in [gjson syntax](https://github.com/tidwall/gjson/blob/master/SYNTAX.md)) to the attribute to patch.
