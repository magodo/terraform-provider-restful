...
- `update_body_patches` (Attributes List) The body patches for update only. Any change here won't cause a plan diff. Note that this is almost only useful for APIs that require *after-create* attribute for an update (e.g. the resource ID). (see [below for nested schema](#nestedatt--update_body_patches))
